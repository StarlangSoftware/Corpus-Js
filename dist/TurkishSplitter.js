(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "./SentenceSplitter", "./Sentence", "nlptoolkit-dictionary/dist/Language/TurkishLanguage", "nlptoolkit-dictionary/dist/Dictionary/Word"], factory);
    }
    ...
    class TurkishSplitter extends SentenceSplitter_1.SentenceSplitter {
        ...
        listContains(currentWord) {
            let shortcuts = ["alb", "bnb", "bkz", "bşk", "co", "dr", "dç", "der", "em", "gn",
         ...
         */
        isNextCharUpperCaseOrDigit(line, i) {
            while (i < line.length && (line.charAt(i) == ' ' || this.SEPARATORS.includes(line.charAt(i)))) {
                i++;
            }
            if (i == line.length || (TurkishLanguage_1.TurkishLanguage.UPPERCASE_LETTERS + TurkishLanguage_1.TurkishLanguage.DIGITS + "-").includes(line.charAt(i))) {
                return true;
            }
            else {
                return false;
         ...
         */
        isPreviousWordUpperCase(line, i) {
            while (i >= 0 && (line.charAt(i) == ' ' || (TurkishLanguage_1.TurkishLanguage.LOWERCASE_LETTERS + "qxw").includes(line.charAt(i)))) {
                i--;
            }
            if (i == -1 || (TurkishLanguage_1.TurkishLanguage.UPPERCASE_LETTERS + "QWX").includes(line.charAt(i))) {
                return true;
            }
            else {
                return false;
        ...
        isNextCharUpperCase(line, i) {
            ...
            if (i == line.length || (TurkishLanguage_1.TurkishLanguage.UPPERCASE_LETTERS + "\"\'").includes(line.charAt(i))) {
                return true;
            }
            else {
                return false;
         ...
         */
        isNameShortcut(currentWord) {
            if (currentWord.length == 1 && TurkishLanguage_1.TurkishLanguage.UPPERCASE_LETTERS.includes(currentWord)) {
                return true;
            }
            if (currentWord.length == 3 && currentWord.charAt(1) == '.' && TurkishLanguage_1.TurkishLanguage.UPPERCASE_LETTERS.includes(currentWord.charAt(2))) {
                return true;
            }
            return false;
        }
         ...
         */
        repeatControl(word, exceptionMode) {
            if (exceptionMode) {
                return word;
            }
            let i = 0;
         ...
         */
        isApostrophe(line, i) {
            let apostropheLetters = TurkishLanguage_1.TurkishLanguage.LETTERS + TurkishLanguage_1.TurkishLanguage.EXTENDED_LANGUAGE_CHARACTERS + TurkishLanguage_1.TurkishLanguage.DIGITS;
            if (i + 1 < line.length) {
        ...
        numberExistsBeforeAndAfter(line, i) {
            if (i + 1 < line.length && i > 0) {
                let previousChar = line.charAt(i - 1);
                let nextChar = line.charAt(i + 1);
                return TurkishLanguage_1.TurkishLanguage.DIGITS.includes(previousChar) && TurkishLanguage_1.TurkishLanguage.DIGITS.includes(nextChar);
            }
            else {
                return false;
            }
        ...
        isTime(line, i) {
            if (i + 2 < line.length) {
                ...
                return TurkishLanguage_1.TurkishLanguage.DIGITS.includes(previousChar) && TurkishLanguage_1.TurkishLanguage.DIGITS.includes(nextChar)
                    && TurkishLanguage_1.TurkishLanguage.DIGITS.includes(twoNextChar);
            }
            else {
                return false;
            }
        ...
        split(line) {
            let emailMode = false;
            let webMode = false;
            let i = 0, specialQuotaCount = 0, roundParenthesisCount = 0, bracketCount = 0, curlyBracketCount = 0, quotaCount = 0, apostropheCount = 0;
            let currentSentence = new Sentence_1.Sentence();
            let currentWord = "";
            ...
            while (i < line.length) {
                if (this.SEPARATORS.includes(line.charAt(i))) {
                    ...
                    if (this.SENTENCE_ENDERS.includes(line.charAt(i))) {
                        ...
                        if (line.charAt(i) == '.' && currentWord != "" && (webMode || emailMode || TurkishLanguage_1.TurkishLanguage.DIGITS.includes(line.charAt(i - 1)))) {
                            currentWord = currentWord + line.charAt(i);
                        }
                        else {
                        ...
                        if (line.charAt(i) == ' ') {
                            ...
                            webMode = false;
                            if (currentWord != "") {
                                currentSentence.addWord(new Word_1.Word(this.repeatControl(currentWord, false)));
                                currentWord = "";
                            }
                        }
                        else {
                            if (line.charAt(i) == '-' && !webMode && roundParenthesisCount == 0 && this.isNextCharUpperCase(line, i + 1) && !this.isPreviousWordUpperCase(line, i - 1)) {
                                if (currentWord != "" && !TurkishLanguage_1.TurkishLanguage.DIGITS.includes(currentWord)) {
                                ...
                                if (this.PUNCTUATION_CHARACTERS.includes(line.charAt(i)) || TurkishLanguage_1.TurkishLanguage.ARITHMETIC_CHARACTERS.includes(line.charAt(i))) {
