...
export class TurkishSplitter extends SentenceSplitter{
    ...
    private isNextCharUpperCaseOrDigit(line: string, i: number): boolean{
        ...
        if (i == line.length || (TurkishLanguage.UPPERCASE_LETTERS + TurkishLanguage.DIGITS + "-").includes(line.charAt(i))) {
            return true;
        } else {
            return false;
        }
    ...
    private isPreviousWordUpperCase(line: string, i: number): boolean{
        while (i >= 0 && (line.charAt(i) == ' ' || (TurkishLanguage.LOWERCASE_LETTERS + "qxw").includes(line.charAt(i)))) {
            i--;
        }
        if (i == -1 || (TurkishLanguage.UPPERCASE_LETTERS + "QWX").includes(line.charAt(i))) {
            return true;
        } else {
            return false;
        }
    ...
    private isNextCharUpperCase(line: string, i: number): boolean{
        ...
        if (i == line.length || (TurkishLanguage.UPPERCASE_LETTERS + "\"\'").includes(line.charAt(i))) {
            return true;
        } else {
            return false;
        }
    ...
    private isNameShortcut(currentWord: string): boolean{
        if (currentWord.length == 1 && TurkishLanguage.UPPERCASE_LETTERS.includes(currentWord)) {
            return true;
        }
        if (currentWord.length == 3 && currentWord.charAt(1) == '.' && TurkishLanguage.UPPERCASE_LETTERS.includes(currentWord.charAt(2))) {
            return true;
        }
        return false;
    }
    ...
    private isApostrophe(line: string, i: number): boolean{
        let apostropheLetters = TurkishLanguage.LETTERS + TurkishLanguage.EXTENDED_LANGUAGE_CHARACTERS + TurkishLanguage.DIGITS;
        if (i + 1 < line.length) {
    ...
    private numberExistsBeforeAndAfter(line: string, i: number){
        if (i + 1 < line.length && i > 0) {
            let previousChar = line.charAt(i - 1);
            let nextChar = line.charAt(i + 1);
            return TurkishLanguage.DIGITS.includes(previousChar) && TurkishLanguage.DIGITS.includes(nextChar);
        } else {
            return false;
        }
    }
    ...
    private isTime(line: string, i: number): boolean{
        if (i + 2 < line.length) {
            ...
            return TurkishLanguage.DIGITS.includes(previousChar) && TurkishLanguage.DIGITS.includes(nextChar)
                && TurkishLanguage.DIGITS.includes(twoNextChar);
        } else {
            return false;
        }
    }
        ...
        while (i < line.length) {
            if (this.SEPARATORS.includes(line.charAt(i))) {
                ...
                if (this.SENTENCE_ENDERS.includes(line.charAt(i))) {
                    ...
                    if (line.charAt(i) == '.' && currentWord != "" && (webMode || emailMode || TurkishLanguage.DIGITS.includes(line.charAt(i - 1)))) {
                        currentWord = currentWord + line.charAt(i);
                    } else {
                    ...
                    if (line.charAt(i) == ' ') {
                        ...
                        if (line.charAt(i) == '-' && !webMode && roundParenthesisCount == 0 && this.isNextCharUpperCase(line, i + 1) && !this.isPreviousWordUpperCase(line, i - 1)) {
                            if (currentWord != "" && !TurkishLanguage.DIGITS.includes(currentWord)) {
                            ...
                            if (this.PUNCTUATION_CHARACTERS.includes(line.charAt(i)) || TurkishLanguage.ARITHMETIC_CHARACTERS.includes(line.charAt(i))) {
